...
const message = (errorCode: number, args: string[] = []) => {
...
const reserveCodeRange = (first: number, last: number, moduleName: string) => {
  assert(first <= last, 'Invalid code range');
  // tslint:disable-next-line:no-any
  const noConflict = reservedCodes.every((range: any) => {
...
export { assert, message, messages, reserveCodeRange };
